...
const SHIELDS = {
    ...
    50:  {label:'BLANC',  color:'#ffffff', max:50},
    75:  {label:'BLEU',   color:'#3c78ff', max:75},
    100: {label:'VIOLET', color:'#b400ff', max:100},
    125: {label:'ROUGE',  color:'#ff1e1e', max:125},
    150: {label:'OR',     color:'#ffd200', max:150},
};

// Distance scale factor (10m = full size, further = smaller target)
const DIST_SCALE = {10:1.0, 25:0.82, 50:0.65, 100:0.45, 200:0.28};

// Optics config
const OPTICS = {
    iron:  {zoom:1,    fov:1,    label:'IRON SIGHTS', crossType:'iron'},
    '1x':  {zoom:1,    fov:1,    label:'1x HOLO',     crossType:'holo'},
    '2x':  {zoom:1.4,  fov:.72,  label:'2x',          crossType:'dot'},
    '3x':  {zoom:2.0,  fov:.5,   label:'3x ACOG',     crossType:'dot'},
    '4x':  {zoom:3.0,  fov:.34,  label:'4-8x SNIPER', crossType:'sniper'},
};
...
function setupOptic(opt){
    if(opt.crossType==='sniper'||opt.crossType==='dot'){
...
function spawnEnemy(){
    ...
    // slight random position around center
    enemy.x = W*0.5 + (Math.random()*120-60);
    enemy.y = H*0.5 + (Math.random()*60-30);
    enemy.scale = sc;
...
function drawCrosshair(cx,cy,rx,ry,col,opticType){
    // The visual crosshair sits at mouse+(recoil) = actual aim point
    const ax=cx+rx, ay=cy+ry;
    const spread=Math.sqrt(rx*rx+ry*ry)*.14;

    ctx.save();
    ctx.strokeStyle=col; ctx.fillStyle=col;
    ctx.lineWidth=1.8; ctx.shadowBlur=5; ctx.shadowColor=col;

    if(opticType==='iron'||opticType==='holo'){
        // Classic 4-line crosshair with gap
        const gap=4, len=9+spread;
        ...
        if(opticType==='holo'){
            ...
            ctx.beginPath();ctx.arc(ax,ay,18+spread*.3,0,Math.PI*2);ctx.stroke();
            ctx.globalAlpha=1;
        }
    } else if(opticType==='dot'){
        // Simple dot
        ctx.beginPath();ctx.arc(ax,ay,2.5,0,Math.PI*2);ctx.fill();
        // Outer ring faint
        ctx.lineWidth=1;ctx.globalAlpha=.3;
        ctx.beginPath();ctx.arc(ax,ay,10+spread*.2,0,Math.PI*2);ctx.stroke();
        ctx.globalAlpha=1;
    } else if(opticType==='sniper'){
        // Drawn on optic canvas — thick mil-dot style
    }
...
function drawSniperOptic(cx,cy,rx,ry){
    ...
    const ax=cx+rx, ay=cy+ry;
    // Dark vignette
    const grad=oCtx.createRadialGradient(cx,cy,H*.28,cx,cy,H*.52);
    grad.addColorStop(0,'rgba(0,0,0,0)');
    grad.addColorStop(1,'rgba(0,0,0,.97)');
    oCtx.fillStyle=grad;
    oCtx.fillRect(0,0,W,H);
    // Circle cutout (already done by gradient — now draw scope reticle)
    oCtx.save();
    oCtx.strokeStyle='rgba(200,200,200,.8)'; oCtx.lineWidth=1;
    // Cross lines
    const r=H*.27;
    oCtx.beginPath();oCtx.moveTo(ax-r,ay);oCtx.lineTo(ax+r,ay);oCtx.stroke();
    oCtx.beginPath();oCtx.moveTo(ax,ay-r);oCtx.lineTo(ax,ay+r);oCtx.stroke();
    // Mil-dot marks
    for(let i=-4;i<=4;i++){if(i===0)continue;
        const d=r/4*i;
        oCtx.beginPath();oCtx.arc(ax+d,ay,2,0,Math.PI*2);oCtx.fillStyle='rgba(200,200,200,.7)';oCtx.fill();
        oCtx.beginPath();oCtx.arc(ax,ay+d,2,0,Math.PI*2);oCtx.fill();
    }
    oCtx.restore();
}
...
function loop(){
    ...
    if(G.gun){
        ...
        else drawCrosshair(mx,my,G.recoilX,G.recoilY,w.color,ot);
